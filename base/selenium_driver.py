...
class SeleniumDriver():
    ...
    def screenShot(self, resultMessage):
        ...
        folder_name = f"screenprints/{folder_timestamp}"
        file_name = f"{resultMessage}_{file_timestamp}.png"

        # Create the screenshot directory if it doesn't exist
        screenshot_directory = os.path.join(os.path.dirname(os.path.dirname(__file__)), folder_name)
        ...
        try:
            ...
            self.log.error(f"### Exception Occurred when taking screenshot: {str(e)}")
            print_stack()
    def getTitle(self):
    ...
    def getElement(self, locator, locatorType="id"):
        ...
        try:
            ...
            element = self.wait.until(EC.element_to_be_clickable((byType, locator)))
    ...
    def getElementList(self, locator, locatorType="id"):
        ...
        elements = self.wait.until(EC.element_to_be_clickable((byType,locator)))
        if len(elements) > 0:
    ...
    def selectDropdown(self, locator, locatorType="id", value=None, visible_text=None):
        """
        Select an option from a dropdown
        :param locator: The locator for the dropdown
        :param locatorType: The type of locator (id, name, xpath, etc.)
        :param value: The value of the option to select (optional)
        :param visible_text: The visible text of the option to select (optional)
        """
        try:
            dropdown_element = self.getElement(locator, locatorType)
            ...
            if value:
                ...
                self.log.error("Either value or visible_text must be provided to select an option from the dropdown.")
            self.log.info(f"Selected option '{visible_text or value}' from dropdown with locator: {locator}")
        except Exception as e:
            self.log.error(f"Error selecting from dropdown: {str(e)}")
            print_stack()
